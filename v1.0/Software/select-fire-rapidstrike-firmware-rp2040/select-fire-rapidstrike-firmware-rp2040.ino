...
RP2040-Zero port of the v1.0 Select-Fire Rapidstrike Kit firmware.

Ported from v1.0/Software/select-fire-rapidstrike-firmware. Control logic is
unchanged from the Nano version; only pin assignments and board-specific
constants were updated for the Waveshare RP2040-Zero. See README.md in this
directory for the full pin map and a list of hardware caveats (logic-level
and gate-drive voltage) that matter if you're adapting the v1.0 PCB to this
MCU rather than designing a new board around it.

Requires the earlephilhower/arduino-pico core (Boards Manager: "Raspberry Pi
Pico/RP2040", board "Waveshare RP2040 Zero"). Library dependencies are the
same as the Nano firmware: JC_Button, CircularBuffer, arduino-timer.
...
#define CURRENT_SENSE_PIN 27		// GP27 / ADC1 (was A1 on Nano)
#define SENSE_RESISTANCE 0.01
// This is the RP2040's ADC reference voltage (ADC_AVDD), which is tied to
// its regulated 3.3V rail regardless of whether the board's VIN/VSYS input
// is fed 5V or 3.3V. Do NOT set this to match VIN/VSYS - it must match the
// ADC's actual reference or current-sense readings will be wrong.
...
void initSerial() {
	Serial.begin(9600);
	// Unlike the Nano's HardwareSerial (always truthy), the RP2040's Serial is
	// native USB CDC and only becomes truthy once a host opens the port.
	// Deliberately NOT blocking here (no `while(!Serial){}`) so the blaster
	// still fires with no PC attached, matching real-world usage.
...
// Sets pusher mechanism based on reading jumper pin (GP11)
...
void handlePusherMotorFiring() {
	// Start motors and fining sequence upon trigger pull when not in safety mode
	if (firingState.wasTriggerPulled) {
	...
	// If cycle control was pressed
	if (firingState.wasCycCtrlSwPressed) {
		firingState.targetPusherState = PUSHER_DRIVE_STATE_BRAKE;

		// Firemode not safety, so probably fire more darts. Brake motors, then
		// check if more darts should be fired. If more darts should be fired, power
		// pusher after a delay (this is dependant on rate of fire).
		if (firingState.currentFireMode != SAFETY) {
			firingState.dartsFired++;

			// Firemode is full auto. Only kill motors if trigger isn't presseed
			if (firingState.currentFireMode == FULL_AUTO) {
				// Trigger isn't pressed, stop firing and brake motors
			...
			// Firemode is semi-atuo. Stop firing and brake motors (because a shot has
			// been fired) if cycle control was pressed
			if (firingState.currentFireMode == SEMI_AUTO) {
				firingState.targetPusherState = PUSHER_DRIVE_STATE_BRAKE;
				firingState.isFiring = false;
			...
			// Firemode is burst. Stop firing and brake motors if all darts have been
			// fired
			if (firingState.currentFireMode == BURST_FIRE) {
				// All darts fired, stop firing and brake
			...
			// If code got to here, there are still more darts to be fired. Power on
			// pusher motor after a slight delay proportional to rate of fire
			updatePusherMotorTiming();
			Serial.println(firingState.rateOfFire);
			firingState.targetPusherState = PUSHER_DRIVE_STATE_BRAKE;
		...
		// Firemode is safety, kill motors
		} else {
			firingState.targetPusherState = PUSHER_DRIVE_STATE_BRAKE;
			firingState.isFiring = false;
		}

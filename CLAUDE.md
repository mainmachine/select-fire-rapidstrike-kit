...
- `v1.0/Software/select-fire-rapidstrike-firmware-nano-hbridge/`: a Nano build driving the pusher motor through an external DRV8871-based H-Bridge motor driver module instead of the v1.0 board's onboard half-bridge FETs. Diverges from the other v1.0 firmware in several load-bearing ways (much shorter cycle-control switch debounce, dart counting on the switch's release edge rather than its press edge, real PWM motor-speed control instead of on/off, H-Bridge-specific FET-transition logic) — see that directory's README for why each of these matters before assuming the "Firmware architecture" section below applies unmodified.
